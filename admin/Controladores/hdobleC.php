...
class HDobleC{
	...
	//Editar HS
	public function EditarHDobleC(){

		$tablaBD = "hdoble";

		$id = "1";

		$respuesta = HDobleM::EditarHDobleM($tablaBD, $id);
	...
	//Actualizar HS
	public function ActualizarHDobleC(){
		...
		if(isset($_POST["precioE"])){
			...
			if(isset($_FILES["imagenE"]["tmp_name"]) && !empty($_FILES["imagenE"]["tmp_name"])){
				...
				if($_FILES["imagenE"]["type"] == "image/jpeg"){
					...
					$rutaImg = "Vistas/img/hd/HD".$nombre.".jpg";
				...
				if($_FILES["imagenE"]["type"] == "image/png"){
					...
					$rutaImg = "Vistas/img/hd/HD".$nombre.".png";
			...
			$respuesta = HDobleM::ActualizarHDobleM($tablaBD, $datosC);

			if($respuesta == true){

				echo '<script>

				window.location = "hdoble";
